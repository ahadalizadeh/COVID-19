...
covid19.data <- function(last.day="03-03-2020"){
  ...
  for (i in 1:length(date)) {
    progress(i, min = 1, max = length(date), 
     ...
     D =try( readr::read_csv(paste0(line,date[i],".csv"),progress =FALSE),silent=TRUE)
     if(class(D)[1]=="try-error") next()
     data[[i]] <- cbind(
      D,
      date = date[i],day = i)
  if("Latitude" %in% names(data[[i]])) data[[i]]$Latitude <- NULL
  if("Longitude" %in% names(data[[i]])) data[[i]]$Longitude <- NULL
     }
...
print("Ahad: You can find data in `COVID19` variable!")
